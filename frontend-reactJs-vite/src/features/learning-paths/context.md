...
- ~~Own pagination controls inlined in `LearningPathsPage`~~ **Closed**: now that a second and third consumer needed the same Prev/Next logic, it was promoted to `shared/components/Pagination.tsx` (see `courses/context.md`) and `LearningPathsPage` was switched to render it instead of its own inline copy — no behavior change, just de-duplication.
