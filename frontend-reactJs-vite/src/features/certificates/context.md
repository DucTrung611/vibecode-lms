...
- ~~No pagination controls on `/my-certificates`~~ **Closed**: `MyCertificatesPage` now lifts `page` state and renders `shared/components/Pagination.tsx` (the component extracted from `courses`' former `CoursePagination` — see `courses/context.md`).
